...
static void
vehicle_webos_init_pdl_locationtracking_callback(struct vehicle_priv *priv, struct callback_list *cbl)
{
	PDL_Err err;
...
static void
vehicle_webos_init_pdl_locationtracking(struct vehicle_priv *priv)
{
	struct callback *cb = NULL;
	struct callback_list *cbl = NULL;

	cbl = callback_list_new();
	cb	= callback_new_2(callback_cast(vehicle_webos_init_pdl_locationtracking_callback),priv,cbl);
...
static void
vehicle_webos_bt_gap_callback(PDL_ServiceParameters *params, void *priv)
{
	...
	if (device_addr) {
		...
		vehicle_webos_init_pdl_locationtracking(priv);
	}
